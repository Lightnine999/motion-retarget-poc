...
def _key_pose_bone(pose_bone, frame: int, previous_euler) -> object:
    # [RSL-patch/Motius-patch] FBX는 회전을 쿼터니언이 아니라 오일러(XYZ) 3채널로만
    # 저장한다. 쿼터니언으로 keyframe을 찍으면 FBX export/import 왕복 시 매 프레임
    # 독립적인 quaternion<->euler 변환이 일어나 랩어라운드(짐벌락)가 생겨 목/어깨 등이
    # 한 프레임 만에 150도 이상 튀는 문제가 발생한다 (2026-09-23 근본 원인 확인).
    #
    # 주의: 여기서 오일러를 뽑는 기준은 반드시 pose_bone.matrix_basis(부모+레스트
    # 기준 "로컬" 변환)여야 한다. 호출부가 pose_bone.matrix(월드/아마추어 공간
    # "절대" 변환)를 넘겨서 그 행렬의 오일러를 그대로 rotation_euler(로컬 필드)에
    # 써버리면, 부모 본(척추 체인 등)이 이미 회전해 있는 프레임에서 그 회전이
    # 자식(목/어깨/팔꿈치)에 중복 반영되어 몸 전체가 뒤틀린다 — 2026-09-23 확인.
    # pose_bone.matrix = full_matrix (setter)는 Blender가 부모의 현재 포즈를 감안해
    # 올바른 matrix_basis를 역산해주므로, 그 결과에서 오일러를 뽑아야 한다.
    pose_bone.rotation_mode = "XYZ"
    local_matrix = pose_bone.matrix_basis
    euler = (
        local_matrix.to_euler("XYZ", previous_euler)
        if previous_euler is not None
        else local_matrix.to_euler("XYZ")
    )
...
def _animate_smpl_armature(armature, payload, fps: float) -> None:
    ...
    # [Motius-patch] matrix를 대입하기 전에 XYZ로 고정해야 한다. 그렇지 않으면
    # Blender가 일단 쿼터니언으로 저장했다가 나중에 오일러로 변환하면서 이미 틀어진
    # 회전 branch가 생기고, 그 뒤 make_compatible()로도 되돌릴 수 없다.
...
def _retarget_animation(target, payload, bone_map, fps: float, root_scale):
    ...
    for pose_bone in target.pose.bones:
        ...
        # [Motius-patch] 이유는 _animate_smpl_armature의 동일 패치 주석 참고.
        pose_bone.rotation_mode = "XYZ"
