...
anim_path = "/Game/Source/SMPL_Source_Anim"
...
root = find_root(source_mesh, "Neck")
bone_order = walk_tree(source_mesh, root)
REPORT["bone_order"] = bone_order

num_frames = unreal.AnimationLibrary.get_num_frames(anim)
THRESH_DEG = 30.0  # 2차 시도(40도) 후에도 131~137, 242 부근에 90~100도급 잔여가 반복 검출됨
...
for frame in range(num_frames):
    poses = unreal.AnimationLibrary.get_bone_poses_for_frame(anim, proper_names_ordered, frame, False, source_mesh)
    for track, t in zip(valid_tracks, poses):
        per_bone_transforms[track].append(t)

controller = anim.get_editor_property("controller")
safe("open_bracket", lambda: controller.open_bracket("Despike source SMPL anim", True))
...
# Unreal 자체 FBX exporter로 내보내기
OUT_PATH = "/Users/kwonkwanggoo/motion-retarget-poc/outputs/kaggle_tennis/smpl_source_clean_unreal_despiked.fbx"
...
export_ok = safe("export_fbx", lambda: unreal.Exporter.run_asset_export_task(task))
REPORT["export_ok"] = export_ok

with open("/Users/kwonkwanggoo/motion-retarget-poc/unreal/scripts/52_report.json", "w") as f:
