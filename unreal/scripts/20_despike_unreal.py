...
for track in track_names:
    rot = safe(f"read_rot_{track}", lambda track=track: list(unreal.AnimationLibrary.get_raw_track_rotation_data(anim, track)))
    pos = safe(f"read_pos_{track}", lambda track=track: list(unreal.AnimationLibrary.get_raw_track_position_data(anim, track)))
    scale = safe(f"read_scale_{track}", lambda track=track: list(unreal.AnimationLibrary.get_raw_track_scale_data(anim, track)))
    if not rot or len(rot) != num_frames:
        continue
...
with open("/Users/kwonkwanggoo/motion-retarget-poc/unreal/scripts/20_report.json", "w") as f:
